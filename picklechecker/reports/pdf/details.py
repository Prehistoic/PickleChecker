...
class PdfDetails:
    ...
    def _build_safety_block(
        ...
        # Safety paragraph
        safety_text = f"<b>Safety: <font color='{safety_color}' size='14'>{result.safety.name}</font></b>"
    ...
    def _build_globals_table(
        ...
        if not result.globals_found:
            block_story.append(Paragraph("No global imports found.", custom_styles["details_value"]))
            return block_story
        ...
        for global_ref in result.globals_found:
            ...
            row = [
                ...
                Paragraph(f"<b><font color='{safety_color}'>{global_ref.safety.name}</font></b>", custom_styles["details_value"])
            ]
            rows.append(row)
        
        # Create table
        globals_table = Table(rows, colWidths=[doc.width * 0.7, doc.width * 0.3])
        globals_table.setStyle(TableStyle([
